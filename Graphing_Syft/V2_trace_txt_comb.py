...
for line in lenstext:
    if j <= 7:  # past the 7th line there is only titles and data.
    ...
    if 8 <= j <= num_lines: # from here on is just data!
...
proc_info = []  # for the csv file to be converted.
print("Starting Trace file..")  # progress report

for line in tracetext:
    proc_info.append(line.split(","))  # this processes every string in the list of tracetext
                                        # and returns the string as a list.

#  these are the titles from the trace file.
trace_titles = proc_info[0]
col1 = 0
offset = len(lens_titles) + 4  # this will allow 4 spaces to the right of the two data sets.

for index, item in enumerate(trace_titles):
    tracetemp = trace_titles[index]
    worksheet.write(col1, (index + offset), tracetemp)

tracetext.seek(0)  # There where problem getting into this next loop.
                   # Not sure why, but the interweb said this would work. IT works :)

for index1, itemlist in enumerate(proc_info):  # this allows each string in the list to be read
    time_pos = proc_info[index1]  # pick one item at a time.
    if index1 > 0:  # Assuming we are past the 1st item in the list everything is Data :)
        for sideside, item2 in enumerate(time_pos):  # this will cycle through the string
            # below will write column per column
            # print(sideside, item2)
            worksheet.write(index1+1, (sideside + offset),  item2)  # writes to the excel worksheet.
        #print(time_pos)
